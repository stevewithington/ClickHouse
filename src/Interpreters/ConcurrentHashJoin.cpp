...
ConcurrentHashJoin::ConcurrentHashJoin(ContextPtr context_, std::shared_ptr<TableJoin> table_join_, size_t slots_, const Block &, const Block & right_sample_block, bool any_take_last_row_)
    : context(context_)
    ...
    , slots(slots_)
{
    if (!slots || slots >= 256)
    {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid argument slot: {}", slots);
    }
